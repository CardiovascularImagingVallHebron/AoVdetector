...
def zoom_central(image_array, zoom_factor=1.5):
    ...
    return cv2.resize(cropped_image, (128, 128), interpolation=cv2.INTER_AREA)
        ...
        if os.path.exists(frame_path):
            for f in os.listdir(frame_path):
                ...
                if p1 == (-1, -1) and p2 == (-1, -1):
                    ...
                    image_resized = cv2.resize(cropped_frame, (128, 128), interpolation=cv2.INTER_AREA)

                # Create the path to save the resized image
                if row['data'] == 'data_psax':
                    save_path = f'data/frames_valve/128_128/{subfolder}/{patient_folder}/{dicom}'
                elif row['data'] == 'data_plax':
                    save_path = f'data_plax/frames_valve/128_128/{subfolder}/{patient_folder}/{dicom}'
                elif row['data'] == 'data_3ch':
                    save_path = f'data_3ch/frames_valve/128_128/{subfolder}/{patient_folder}/{dicom}'
...
if __name__ == "__main__":
    ...
    df = pd.read_csv('inference_output_48m_corrected_upd_2_128.csv') 
